...
impl Solution {
    pub fn is_alien_sorted(words: Vec<String>, order: String) -> bool {
        ...
        fn compare(first: &str, second: &str, dict: &HashMap<u8, usize>) -> bool {
            ...
            dict.get(&first[idx..=idx].as_bytes()[0]).unwrap() < dict.get(&second[idx..=idx].as_bytes()[0]).unwrap()
        }

        let dict = order.bytes().enumerate().map(|(idx, byte)| (byte, idx)).collect::<HashMap<u8, usize>>();
